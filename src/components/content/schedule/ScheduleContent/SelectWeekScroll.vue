    ...
    >
      ...
      >
        <view
          ...
          :style="{
            backgroundColor: getPickWeek == index ? getThemeColor.curBgSecond : getThemeColor.curBg,
          }"
          >{{ index + 1 }} 周</view
        >
      </view>
...
export default {
  setup() {
    ...
    const changeSelectWeek = (event, index) => {
      ...
      swiperList.value[getCurrentSwiperIndex.value] = weeksData.value[getPickWeek.value]
      swiperList.value[(getCurrentSwiperIndex.value + 1) % 3] = weeksData.value[(getPickWeek.value + 1) % 20]
