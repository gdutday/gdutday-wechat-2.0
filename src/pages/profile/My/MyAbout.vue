<template>
  <view>
    ...
    <view class="version-choose pt-3">
      <view
        v-for="(item, index) of version"
        :key="index"
        class="version-choose-item py-2"
        :class="{ active: index == currentChoose }"
        @tap="chooseVer(index)"
        >{{ item }}</view
      >
    </view>
    <view class="w-1 px-2">
      ...
      <ming-container class="w-1 p-3" v-else>
...
export default {
  ...
  setup() {
    const version = ["1.0版本", "2.0版本"];
    let currentChoose = ref(1);
