    ...
    <view class="px-3 w-1">
      <ming-container class="w-1 p-3">
        ...
        <template v-slot:default>
          <view class="w-1">
            <view class="ts-desc w-1 px-2 my-2" :style="{borderBottom: `${getThemeColor.curBg} 2px solid`}"
            ...
            <view class="ts-theme w-1 my-2">
              <view v-for="(value, key) in color" :key="key" @click="setTheme(key, value)"
                class="ts-theme-item flex-center">
                <view class="ts-theme-item-show" :style="{
                  backgroundImage: `linear-gradient(90deg, ${value.bgColor} ,${'#ccc'})`,
                  marginTop: '20rpx',
                }">
                  <text class="t iconfont icon-icon-test45" :class="isChange ? 'animation-fade' : ''"
                    v-if="value.bgColor == getThemeColor.curBg"></text>
                </view>
              </view>
            </view>
            <ming-list-item content="是否要上传背景图片" correctContent="上传" @cancelMethod="removeBackgroundImage"
              @correctMethod="openModal" :themeColor="getThemeColor">
            </ming-list-item>
            <!-- <ming-list-item
            ...
            <ming-list-item content="透明度设置" :themeColor="getThemeColor">
              <slider class="p-0 m-0" :style="{width: '120px'}" min="0" max="100" :value="getOpacity" step="1"
                :activeColor="getThemeColor.curBgSecond" :block-color="getThemeColor.curBgSecond" @change="sliderChange"
                show-value />
            </ming-list-item>
          </view>
        </template>
  ...
  setup() {
    ...
    const choosePgPic = async e => {
      ...
      try {
        ...
        const {savedFilePath: newPath} = await becomePromise(wx.getFileSystemManager().saveFile, {tempFilePath: path}, 'saveFile')
